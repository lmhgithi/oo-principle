...
class SuperParkingBoyTest {
  ...
  @Test
  void should_pick_up_car_when_only_one_parking_lot_has_most_available_rate() {
    this.smartParkingBoy = new SmartParkingBoy(new ArrayList<ParkingLot>() {{
      add(new NormalParkingLot(10));
      add(new NormalParkingLot(100));
      add(new NormalParkingLot(1000));
    }});

    Ticket ticket = smartParkingBoy.park(new Car());

    assertThat(ticket).isNotEqualTo(null);
    assertThat(getAvailableSpaceOfXParkingLot(0)).isEqualTo(10);
    assertThat(getAvailableSpaceOfXParkingLot(1)).isEqualTo(100);
    assertThat(getAvailableSpaceOfXParkingLot(2)).isEqualTo(999);
  }
